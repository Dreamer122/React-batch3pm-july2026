...
const Registration = () => {
          ...
          if (n=="first_name"){
            if(!val.match("^[A-Za-z\\s]+$")){
              setError((prev)=>{
                return {
                  ...prev,
                  'ferror':"only alphabets are allowed"
                }
              })
            }
            else{
             ...
             if(!val.match("^[A-Za-z\\s]+$")){
              setError((prev)=>{
                return {
                  ...prev,
                  'lerror':"only alphabets are allowed"
                }
              })
            }
            else{
   ...
   const submitHandle=(e)=>{
    e.preventDefault()
    const hasError=Object.values(error).some((v)=>v!=="") // True , false
  ...
  <form onSubmit={(e)=>submitHandle(e)} action=""  className="mx-auto mt-16 max-w-xl sm:mt-20">
    <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
      <div>
        <label htmlFor="first-name" className="block text-sm/6 font-semibold text-gray-900">First name</label>
        <div className="mt-2.5">
          <input onChange={(e)=>getvalue(e.target)} id="first-name" type="text" name="first_name" autoComplete="given-name" className={`block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 ${error.ferror?'outline-red-500':'outline-gray-300'}  placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600` } />
      {/* ternary operator
      conidtional rendering
      */}
       {error.ferror?<span className='text-red-600'>{error?.ferror}</span>:""} 
        </div>
      </div>
      <div>
        <label htmlFor="last-name" className="block text-sm/6 font-semibold text-gray-900">Last name</label>
        <div className="mt-2.5">
          <input onChange={(e)=>getvalue(e.target)} id="last-name" type="text" name="last_name" autoComplete="family-name" className="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600" />
       {error.lerror?<span className='text-red-600'>{error?.lerror}</span>:""} 
      ...
      <div className="sm:col-span-2">
        ...
        <div className="mt-2.5">
          <input onChange={(e)=>getvalue(e.target)} id="company" type="text" name="company" autoComplete="organization" className="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600" />
        </div>
      </div>
      <div className="sm:col-span-2">
        <label htmlFor="email" className="block text-sm/6 font-semibold text-gray-900">Email</label>
        <div className="mt-2.5">
          <input onChange={(e)=>getvalue(e.target)} id="email" type="email" name="email" autoComplete="email" className="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600" />
       {error.emailerror?<span className='text-red-600'>{error?.emailerror}</span>:""} 
      ...
      <div className="sm:col-span-2">
        ...
        <div className="mt-2.5">
          <div className="flex rounded-md bg-white outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
            ...
            <input onChange={(e)=>getvalue(e.target)} id="phone-number" type="text" name="phone-number" placeholder="123-456-7890" className="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" />
          </div>
        </div>
      </div>
      <div className="sm:col-span-2">
        ...
        <div className="mt-2.5">
          <textarea onChange={(e)=>getvalue(e.target)} id="message" name="message" rows="4" className="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600"></textarea>
        </div>
      </div>
